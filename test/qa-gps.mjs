...
// gx:Track porte les coordonnées en <gx:coord> ; <coordinates> n'apparaît
// qu'avec une route prévue (aucun plan chargé en QA).
kmlFile && kml.includes('<kml') && kml.includes('<gx:coord>')
    ? ok(`export KML valide (${path.basename(kmlFile)}, gx:Track + altitudes)`) : ko('KML invalide ou absent : ' + (kmlFile || 'rien téléchargé') + ' / ' + kml.slice(0, 120));
