...
//   → Apercu_Log-nav_Vannes-Cherbourg.pdf + test/fixtures-navlog-vannes-cherbourg.json
...
const ROUTE = ['LFRV', 'LFRP', 'LFRW', 'LFOM', 'LFRC'];
...
fs.writeFileSync(path.join(root, 'test', 'fixtures-navlog-vannes-cherbourg.json'), JSON.stringify(sample, null, 1));
const doc = drawNavLogPdf(jsPDF, sample);
const out = path.join(root, 'Apercu_Log-nav_Vannes-Cherbourg.pdf');
